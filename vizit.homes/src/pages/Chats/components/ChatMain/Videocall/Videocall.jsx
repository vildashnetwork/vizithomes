...
const VideoCallPage = ({ remoteUserId, remoteUserName, setiscall }) => {
    const socketRef = useRef(null);
    const peerRef = useRef(new PeerService());
    ...
    /* SOCKET SETUP (PER PAGE INSTANCE) */
    useEffect(() => {
        socketRef.current = io(SOCKET_SERVER_URL, {
            query: { userId: localStorage.getItem("userId") },
        });
    ...
    useEffect(() => {
        ...
        peer.setTrackHandler(setRemoteStream);

        peer.setIceStateHandler((state) => {
    ...
    useEffect(() => {
        ...
        const onIncoming = async ({ fromUserId, offer, callerName }) => {
            setCallerInfo({ userId: fromUserId, name: callerName });
            setIncomingCall(true);
            await prepare();
            await peer.peer.setRemoteDescription(JSON.parse(offer));
        };

        s.on("incoming:call", onIncoming);

        s.on("call:accepted", async ({ answer }) => {
            await peer.setRemoteAnswer(JSON.parse(answer));
        });
    ...
    const acceptCall = async () => {
        const answer = await peerRef.current.getAnswer();
        socketRef.current.emit("call:accepted", {
            toUserId: callerInfo.userId,
            answer: JSON.stringify(answer),
        });
        ...
        <div className="video-call-container">
            ...
            {callActive && (
                <div className="video-call-interface">
                    <div className="remote-video-container">
                        {largeStream && (
                            <VideoPlayer stream={largeStream} name={largeName} muted={isSwapped} />
                        )}
                    </div>
                    ...
                    <div className="call-controls">
                        <button className="end-call-btn" onClick={toggleMute}>
                            {isMuted ? <MicOffIcon /> : <MicIcon />}
                        </button>

                        <button className="end-call-btn" onClick={toggleVideo}>
                            {isVideoOff ? <VideocamOffIcon /> : <VideocamIcon />}
                        </button>
